...
/**
 * 주문 취소 시 매장 담당자 메일 HTML (신규 주문 메일과 동일 레이아웃, 문구·취소 표시만 변경)
 */
function buildOrderCancellationHtml(order, stores) {
  const slugToBrandName = buildSlugToBrandName(stores);
  const getBrandName = (slug) => slugToBrandName[slug] || slugToBrandName[String(slug || '').toLowerCase()] || slug || '기타';
  ...
  const orderId = escapeHtml(order.id || '');
  const createdAt = formatOrderDate(order.created_at || order.createdAt);
  const cancelReasonRaw = (order.cancel_reason || order.cancelReason || '').toString().trim();
  const cancelReasonEsc = escapeHtml(cancelReasonRaw || '—');
    ...
    rowsHtml += `
      ...
      ${items
        .map(
          (i) => `
      <tr>
        <td style="padding:8px 12px; border-bottom:1px solid #f0f0f0; ${STRIKE}">${escapeHtml(i.name)}</td>
        <td style="padding:8px 12px; border-bottom:1px solid #f0f0f0; text-align:right; ${STRIKE}">${formatPrice(i.price)} × ${i.qty}</td>
        <td style="padding:8px 12px; border-bottom:1px solid #f0f0f0; text-align:right; ${STRIKE}">${formatPrice(i.price * i.qty)}</td>
      </tr>`
        )
        .join('')}
      <tr><td colspan="2" style="padding:6px 12px; text-align:right; font-weight:600; ${STRIKE}">소계</td><td style="padding:6px 12px; text-align:right; ${STRIKE}">${formatPrice(catTotal)}</td></tr>
    `;
  }
...
<body style="margin:0; padding:0; font-family:'Noto Sans KR', -apple-system, BlinkMacSystemFont, sans-serif; font-size:14px; line-height:1.5; color:#333;">
  <div style="max-width:600px; margin:0 auto; padding:24px;">
    <h2 style="margin:0 0 8px; font-size:18px; color:#e67b19;">BzCat 주문 취소 안내</h2>
    <p style="margin:0 0 20px; color:#666;">아래 주문이 취소되었습니다. 취소 사유 : ${cancelReasonEsc}</p>
    ...
    <table style="width:100%; border-collapse:collapse; margin-bottom:20px; background:#fff; border:1px solid #e0e0e0; border-radius:8px; overflow:hidden;">
      ...
      ${rowsHtml}
      <tr style="background:#f8f9fa;">
        <td colspan="2" style="padding:12px; font-weight:700; ${STRIKE}">총 결제 금액</td>
        <td style="padding:12px; text-align:right; font-weight:700; font-size:16px; color:#e67b19; ${STRIKE}">${formatPrice(totalAmount)}</td>
      </tr>
    </table>

    <table style="width:100%; border-collapse:collapse; margin-bottom:20px; background:#fff; border:1px solid #e0e0e0; border-radius:8px; overflow:hidden;">
      <tr><td colspan="2" style="padding:12px; background:#f8f9fa; font-weight:600; border-bottom:1px solid #e0e0e0;">취소 주문 정보</td></tr>
      <tr><td style="padding:10px 12px; width:120px; border-bottom:1px solid #eee;">주문번호</td><td style="padding:10px 12px; border-bottom:1px solid #eee;">#${orderId}</td></tr>
      <tr><td style="padding:10px 12px; border-bottom:1px solid #eee;">주문일시</td><td style="padding:10px 12px; border-bottom:1px solid #eee;">${createdAt}</td></tr>
      <tr><td style="padding:10px 12px; border-bottom:1px solid #eee;">주문자명</td><td style="padding:10px 12px; border-bottom:1px solid #eee;">${depositor}</td></tr>
      <tr><td style="padding:10px 12px; border-bottom:1px solid #eee;">배송 희망일</td><td style="padding:10px 12px; border-bottom:1px solid #eee;">${deliveryDate} ${deliveryTime}</td></tr>
