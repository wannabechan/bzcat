...
async function generateOrderPdf(order, stores = [], options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const notices = [
      ...
      '· 홈페이지 \'내 주문 보기\'에서 해당 주문의 [결제 링크 보기] 버튼을 누르시고 결제를 진행하시면 됩니다.',
    ...
    function drawTableHeader() {
      ...
      doc.fillColor('#000').fontSize(9);
      if (!useKorean) doc.font('Helvetica-Bold');
      const headerCenterY = y + 24 / 2 + 9 * 0.35;
      doc.text('메뉴명', col1, headerCenterY);
      doc.text('수량', col2, headerCenterY);
      doc.text('단가', col3, headerCenterY);
      doc.text('금액', col4, headerCenterY);
      if (useKorean) doc.font('NotoSansKR');
      y += 24;
      drawHLine(y, '#ccc');
    }
    ...
    for (const slug of orderedSlugs) {
      ...
      const catCenterY = y + 20 / 2 + 10 * 0.35;
      doc.text(`[${title}]`, col1, catCenterY);
      ...
      for (const item of byCategory[slug]) {
        ...
        const itemCenterY = y + 18 / 2 + 9 * 0.35;
        doc.text(`- ${String(item.name || '')}`, col1, itemCenterY, { width: col2 - col1 - 8 });
        doc.text(String(item.qty || 0), col2, itemCenterY);
        doc.text(formatPrice(item.price || 0), col3, itemCenterY);
        doc.text(formatPrice(lineTotal), col4, itemCenterY, { width: 55, align: 'right' });
        y += rowH;
        drawHLine(y, '#eee');
        rowNum++;
      }
    ...
    doc.fontSize(10).fillColor('#000');
    if (!useKorean) doc.font('Helvetica-Bold');
    const totalCenterY = y + 28 / 2 + 10 * 0.35;
    doc.text('총 주문 금액', col1, totalCenterY);
    doc.fontSize(9);
    doc.text(formatPrice(order.total_amount || 0), col4, totalCenterY, { width: 55, align: 'right', lineBreak: false });
    if (useKorean) doc.font('NotoSansKR');
    y += 28;
    drawHLine(y, '#ccc');
